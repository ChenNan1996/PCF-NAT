...
# reference: PCF-ECAPA-TDNN https://ieeexplore.ieee.org/document/10095051
...
from models.Modules import TDNNBlock, BatchNorm1d, get_posemb2 as get_posemb, DropPath
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, bias=True, groups=1, act_layer=nn.GELU, drop=0.):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.fc1 = nn.Conv1d(in_features, hidden_features, kernel_size=1, bias=bias, groups=groups)
        self.act = act_layer()
        self.fc2 = nn.Conv1d(hidden_features, out_features, kernel_size=1, bias=bias, groups=groups)
...
class NAttention(nn.Module):
    def __init__(self, frames, dim, num_heads, win_size, dilation=1, groups=1, proj_bias=True, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., dtype=torch.float32):
        super().__init__()
        ...
        self.qkv = nn.Conv1d(dim, dim * 3, kernel_size=1, bias=qkv_bias, groups=groups)
        ...
        self.proj = nn.Conv1d(dim, dim, kernel_size=1, bias=proj_bias, groups=groups)
    ...
    def forward(self, x, padding=None):
        ...
        x = self.qkv(x) # ( N, C*3, T )
        x = x.view(N, C//self.groups, self.groups, 3, T)
        x = x.transpose(1,2) # (N, groups, C//groups, 3, T)
        x = x.contiguous().view(N, num_heads, head_dim, 3, T)
        ...
        if x.device.type=='cuda':
            x = x.permute(3,0,1,4,2) # ( 3, N, num_heads, T, head_dim )
            x = x.contiguous().view(3, N*num_heads, T, head_dim)
            ...
            x = x.transpose(2,3).contiguous().view(N,C,T)
        else:
            right = win_size-left-1
            x = x.permute(3,0,4,1,2) # ( 3, N, T, num_heads, head_dim )
            ...
            x = x.squeeze(-2).permute(0,2,3,1).contiguous().view(N,C,T)
...
class GlobalAttention(nn.Module):
    def __init__(self, dim, num_heads, groups=1, proj_bias=True, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        self.groups = groups
        ...
        self.linear_pos = nn.Conv1d(dim, dim, kernel_size=1, bias=False, groups=groups)
        self.qkv = nn.Conv1d(dim, dim * 3, kernel_size=1, bias=qkv_bias, groups=groups)
    ...
    def forward(self, x, pos_emb, attn_mask=None):
        ...
        x = x.view(N, C//self.groups, self.groups, 3, T)
        x = x.transpose(1,2) # (N, groups, C//groups, 3, T)
        x = x.contiguous().view(N, self.num_heads, C//self.num_heads, 3, T)
        x = x.permute(3,0,1,4,2) # (3, N, num_heads, T, head_dim)
...
class NATFLayer(nn.Module):
    def __init__(
        self, frames, dim, num_heads, win_size, dilation=1, groups=1,
        ...
        #assert groups<=num_heads and num_heads%groups==0
        self.win_size = win_size
        if isinstance(win_size, list) or win_size>0:
            self.local = True
            self.attn = NAttention(frames, dim, num_heads, win_size, dilation=dilation, groups=groups, proj_bias=bias, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, dtype=dtype)
        else:
            self.local = False
            assert dilation==1
            self.attn = GlobalAttention(dim, num_heads, groups=groups, proj_bias=bias, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
        self.norm1 = norm_layer(dim)
        
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, groups=groups, act_layer=act_layer, drop=drop)
...
class NATFBlock(nn.Module):
    def __init__(self, frames, depth, dim, num_heads, win_size, dilation=1, groups=1,
        ...
        assert isinstance(groups, int) or (isinstance(groups, list) and len(groups)==depth)
...
class PatchEmbed(nn.Module):
    def __init__(self, in_chans, embed_dim, patch_size=1, groups=1, norm_layer=None):
        super().__init__()
        self.proj = nn.Conv1d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size, groups=groups)
...
class PCF_NATransformer(nn.Module):
    def __init__(
        ...
        win_size=[[27]*4,[27,27,27,0],[27]*4,[27,27,27,0]], dilation=1, groups=[8,4,2,1],
        ...
        assert isinstance(groups, int) or (isinstance(groups, list) and len(groups)==num_blocks)
        ...
        for i in range(num_blocks):
            self.patch_embeds.append(PatchEmbed(in_chans=in_chans, embed_dim=embed_dim, patch_size=patch_size, groups=groups[i], norm_layer=nn.BatchNorm1d if patch_norm else None))
        ...
        for i in range(num_blocks):
            block = NATFBlock(
                ...
                norm_layer=nn.BatchNorm1d, dtype=dtype,
    ...
    def forward(self, x, lens=None):#input: N, F, T
        ...
        T = ( T-self.patch_size ) // self.patch_size +1
        win_size = self.win_size
        ...
        if self.attn_global:
            if self.training:
                ...
                if self.pos_emb_eval is None:
                    self.pos_emb_eval = get_posemb(dim=self.embed_dim, max_len=40000).to(x.device)
        ...
        if self.mfa:
            ...
            for i in range(self.num_blocks):
                x1 = self.patch_embeds[i](x)
                xi = self.blocks[i](x1 if i==0 else x1+xi, padding=padding, pos_emb=pos_emb, attn_mask_global=attn_mask_global) # N, C, T//2
                xs.append(xi)
            x = torch.cat(xs, dim=1) # N, C*num_blocks, T//2
        else:
            for i in range(self.num_blocks):
                x1 = self.patch_embeds[i](x)
                xi = self.blocks[i](x1 if i==0 else x1+xi, padding=padding, pos_emb=pos_emb, attn_mask_global=attn_mask_global) # N, C, T//2
