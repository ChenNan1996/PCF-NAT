...
__global__ void ana_q_backward_kernel(__half2 *__restrict__ query_gd,
    ...
    __half *shares = (__half *)shares8;
    __half *shares_attn_gd = (__half *)(shares8+(left+tile_size+right));
    const int part = min(tile_size, T-blockIdx.y*tile_size);
    {
        ...
        for(int j=0; j<ceil((float)tile_size*win_size/32/blockDim.y); j++){
            const int index = blockDim.y*j*32 + 32*threadIdx.y + threadIdx.x;
            if(index<part*win_size){ shares_attn_gd[index/win_size*win_size + index%win_size] = __float2half(attn_gd[ntk+index]); }
        }
        //__syncthreads();
    }
    ...
    for(int c=0; c<C; c+=WMMA_N){
        ...
        for(int i=0; i<ceil((float)win_size/WMMA_K)+1; i++){//+1 is WMMA_M/WMMA_K
            ...
            {
                __half *RA_fp16 = (__half *)RA;
                const int k = i*WMMA_K + col - row;
                if(t<T){
                    if( k>=0 && k<win_size ){ RA_fp16[0]=shares_attn_gd[t_tile*win_size+k]; }
                    if( k>=-1 && k+1<win_size ){ RA_fp16[1]=shares_attn_gd[t_tile*win_size+k+1]; }
                    if( k>=-8 && k+8<win_size ){ RA_fp16[4]=shares_attn_gd[t_tile*win_size+k+8]; }
                    if( k>=-9 && k+9<win_size ){ RA_fp16[5]=shares_attn_gd[t_tile*win_size+k+9]; }
                }
                if(t+8<T){
                    if( k>=8 && k-8<win_size ){ RA_fp16[2]=shares_attn_gd[(t_tile+8)*win_size+k-8]; }
                    if( k>=7 && k-7<win_size ){ RA_fp16[3]=shares_attn_gd[(t_tile+8)*win_size+k-7]; }
                    if( k>=0 && k<win_size ){ RA_fp16[6]=shares_attn_gd[(t_tile+8)*win_size+k];}
                    if( k>=-1 && k+1<win_size ){ RA_fp16[7]=shares_attn_gd[(t_tile+8)*win_size+k+1]; }
                }
            }
            uint32_t RB[2] = {0,0};
            {
                __half *RB_fp16 = (__half *)RB;
                const int t1 = threadIdx.y*WMMA_M+i*WMMA_K+col;
                const int t2 = t1 -(left+part+right);
                if( t2<0 ){ RB_fp16[0]=shares[t1*8+row]; }
                if( t2<-1 ){ RB_fp16[1]=shares[(t1+1)*8+row]; }
                if( t2<-8 ){ RB_fp16[2]=shares[(t1+8)*8+row]; }
                if( t2<-9 ){ RB_fp16[3]=shares[(t1+9)*8+row]; }
            }
        ...
        if( t<T ){ query_gd[((blockIdx.x*T+blockIdx.y*tile_size+t_tile)*C + c+col)/2] = __float22half2_rn(acc[0]); }
        if( t+8<T ){ query_gd[((blockIdx.x*T+blockIdx.y*tile_size+t_tile+8)*C + c+col)/2] = __float22half2_rn(acc[1]); }
    }
}


__global__ void ana_k_backward_kernel(__half2 *__restrict__ key_gd,
                            const float *__restrict__ attn_gd,
                            const uint128_t *__restrict__ query,
                            const int N,
                            const int T,
    ...
    const int part = min(tile_size, T-blockIdx.y*tile_size);
    {
        ...
        for(int j=0; j<ceil((float)(part+win_size-1)*win_size/32/blockDim.y); j++){
            const int index = blockDim.y*j*32 + 32*threadIdx.y + threadIdx.x;
            if(start0+index>=0 && start0+index<T*win_size){ shares_attn_gd[index/win_size*win_size + index%win_size] = __float2half(attn_gd[start+index]); }
        }
        //__syncthreads();
    }
    ...
    for(int c=0; c<C; c+=WMMA_N){
        ...
        for(int i=0; i<ceil((float)win_size/WMMA_K)+1; i++){
            uint32_t RA[4] = {0,0,0,0};
            {
                ...
                if( t1>=0 && t1<T && k>=0 && k<win_size ){ RA_fp16[0]=shares_attn_gd[t_tile1*win_size+k]; }
                if( t1>=-1 && t1+1<T && k>=1 && k-1<win_size ){ RA_fp16[1]=shares_attn_gd[(t_tile1+1)*win_size+k-1]; }
                if( t1>=-8 && t1+8<T && k>=8 && k-8<win_size ){ RA_fp16[4]=shares_attn_gd[(t_tile1+8)*win_size+k-8]; }
                if( t1>=-9 && t1+9<T && k>=9 && k-9<win_size ){ RA_fp16[5]=shares_attn_gd[(t_tile1+9)*win_size+k-9]; }
                
                if( t1>=0 && t1<T && k>=-8 && k+8<win_size ){ RA_fp16[2]=shares_attn_gd[t_tile1*win_size+k+8]; }
                if( t1>=-1 && t1+1<T && k>=-7 && k+7<win_size ){ RA_fp16[3]=shares_attn_gd[(t_tile1+1)*win_size+k+7]; }
                if( t1>=-8 && t1+8<T && k>=0 && k<win_size ){ RA_fp16[6]=shares_attn_gd[(t_tile1+8)*win_size+k]; }
                if( t1>=-9 && t1+9<T && k>=1 && k-1<win_size ){ RA_fp16[7]=shares_attn_gd[(t_tile1+9)*win_size+k-1]; }
            }
        ...
        float2 *acc = (float2 *)RAcc;
        if( t<T ){ key_gd[((blockIdx.x*T+blockIdx.y*tile_size+t_tile)*C + c+col)/2] = __float22half2_rn(acc[0]); }
        if( t+8<T ){ key_gd[((blockIdx.x*T+blockIdx.y*tile_size+t_tile+8)*C + c+col)/2] = __float22half2_rn(acc[1]); }        
    }
...
void fp16_ana_qk_forward_C16(float *attn,
    ...
    const int len1 = ceil((float)(tile_size+left+win_size-left-1+4)/8)*8-4;
    const int size = len1*WMMA_K*sizeof(__half) + tile_size*win_size*sizeof(float);
    //printf("\t[49152-size=%d]\n", 49152-size);
...
void fp16_ana_qk_forward_C8(float *attn,
    ...
    const int size = (tile_size+win_size-1)*WMMA2_K*sizeof(__half) + tile_size*win_size*sizeof(float);
    //printf("\t[49152-size=%d]\n", 49152-size);
...
void fp16_ana_q_backward(__half *query_gd,
    ...
    const int size = len1*8*sizeof(__half) + tile_size*win_size*sizeof(__half);
    ana_q_backward_kernel<<<gridSize, blockSize, size>>>((__half2 *)query_gd, attn_gd, (uint128_t *)key, N, T, C, win_size, left, tile_size, win_size-left-1);
}
...
void fp16_ana_k_backward(__half *key_gd,
    ...
    const int len1 = tile_size + win_size-1;
    const int size = len1*8*sizeof(__half) + len1*win_size*sizeof(__half);
    ana_k_backward_kernel<<<gridSize, blockSize, size>>>((__half2 *)key_gd, attn_gd, (uint128_t *)query, N, T, C, win_size, win_size-left-1, tile_size, left);
}
